...
export function MetricsLibrary({ metrics }: { metrics: Metric[] }) {
  const [query, setQuery] = React.useState("");
  const [layer, setLayer] = React.useState<MetricLayer | typeof ALL_LAYERS>(ALL_LAYERS);
  const [source, setSource] = React.useState<SourceType | typeof ALL_SOURCES>(ALL_SOURCES);

  const countsByLayer = React.useMemo(
    () =>
      Object.fromEntries(
        METRIC_LAYERS.map((item) => [item, metrics.filter((metric) => metric.layer === item).length]),
    ...
    <div className="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
      <aside className="space-y-4">
        <section className="rounded-lg border border-[#e6e6e6] bg-white p-4 shadow-sm">
          ...
          <h1 className="mt-3 text-[24px] font-semibold leading-[1.05] tracking-[-0.04em] text-[#1e1e1e]">
            Biblioteca de métricas
          </h1>
          <p className="mt-2 text-sm leading-[1.6] text-[#626262]">
        ...
        <section className="rounded-lg border border-[#e6e6e6] bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 space-y-1.5">
            <button
              ...
              className={`flex w-full items-center justify-between rounded-md px-2.5 py-2 text-left text-sm transition-[background-color,color,border-color] ${
                layer === ALL_LAYERS
            ...
            {METRIC_LAYERS.map((item) => (
              ...
              >
                <span className="max-w-[180px] leading-tight">{layerLabels[item]}</span>
                <span className="text-xs">{countsByLayer[item]}</span>
              </button>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-lg border border-[#e6e6e6] bg-white p-4 shadow-sm">
          <div className="space-y-3">
            <label className="block">
              <span className="mb-1.5 block text-[10px] font-medium uppercase tracking-[0.08em] text-[#757575]">
                Buscar
              </span>
              <span className="flex items-center gap-2 rounded-md border border-[#e6e6e6] bg-[#f7f7f7] px-3 py-2">
                <Search className="h-4 w-4 text-[#949494]" />
                <input
                  value={query}
                  onChange={(event) => setQuery(event.target.value)}
                  placeholder="Nombre o tag"
                  className="w-full bg-transparent text-sm text-[#1e1e1e] outline-none placeholder:text-[#949494]"
                />
              </span>
            </label>
            ...
            <label className="block">
              ...
              <select
                value={layer}
                onChange={(event) =>
                  setLayer(event.target.value as MetricLayer | typeof ALL_LAYERS)
                }
                className="w-full rounded-md border border-[#e6e6e6] bg-white px-3 py-2 text-sm text-[#1e1e1e] outline-none focus:border-[#0d99ff] focus:ring-2 focus:ring-[#0d99ff]/15"
              >
            ...
            <label className="block">
              ...
              <select
                value={source}
                onChange={(event) =>
                  setSource(event.target.value as SourceType | typeof ALL_SOURCES)
                }
                className="w-full rounded-md border border-[#e6e6e6] bg-white px-3 py-2 text-sm text-[#1e1e1e] outline-none focus:border-[#0d99ff] focus:ring-2 focus:ring-[#0d99ff]/15"
              >
        ...
        <section className="rounded-lg border border-[#e6e6e6] bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            <div className="rounded-md border border-[#ececec] bg-[#fafafa] px-3 py-2">
              <p className="text-[10px] uppercase tracking-[0.08em] text-[#757575]">
                Total
              </p>
              <p className="mt-1 text-lg font-semibold tracking-[-0.03em] text-[#1e1e1e]">
                {metrics.length}
              </p>
            </div>
            <div className="rounded-md border border-[#ececec] bg-[#fafafa] px-3 py-2">
